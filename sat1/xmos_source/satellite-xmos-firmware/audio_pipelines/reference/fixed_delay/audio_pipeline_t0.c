...
#else
#if appconfAUDIO_PIPELINE_CHANNELS >= 4
    int32_t DWORD_ALIGNED beam_output[appconfAUDIO_PIPELINE_FRAME_ADVANCE];
    for (int i = 0; i < appconfAUDIO_PIPELINE_FRAME_ADVANCE; i++) {
        int64_t sum = 0;
        for (int ch = 0; ch < appconfAUDIO_PIPELINE_CHANNELS; ch++) {
            sum += frame_data->samples[ch][i];
        }
        beam_output[i] = saturate_i32(sum / appconfAUDIO_PIPELINE_CHANNELS);
    }

    memcpy(frame_data->samples[0], beam_output, appconfAUDIO_PIPELINE_FRAME_ADVANCE * sizeof(int32_t));
#if appconfAUDIO_PIPELINE_STORE_IC_AUDIO
    memcpy(frame_data->aec_reference_audio_samples[0], beam_output, appconfAUDIO_PIPELINE_FRAME_ADVANCE * sizeof(int32_t));
#endif
