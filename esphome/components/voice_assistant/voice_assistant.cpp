  ...
  switch (msg.event_type) {
    ...
    case api::enums::VOICE_ASSISTANT_TTS_START: {
      ...
      for (const auto &arg : msg.data) {
        if (arg.name == "text") {
          text = arg.value;
        } else if (arg.name == "tts_kind") {
          is_tool_call = arg.value == "tool";
        }
      }
      if (text.empty()) {
      ...
      if (is_tool_call) {
        ESP_LOGD(TAG, "Tool call: \"%s\"", text.c_str());
      } else {
        ESP_LOGD(TAG, "Response: \"%s\"", text.c_str());
      }
      this->defer([this, text, is_tool_call]() {
        if (is_tool_call) {
          this->tool_call_start_trigger_.trigger(text);
        } else {
