...
CONF_ON_TOOL_CALL_START = "on_tool_call_start"
    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_ON_TOOL_CALL_START): automation.validate_automation(
                single=True
            ),
...
async def to_code(config):
    ...
    if CONF_ON_TOOL_CALL_START in config:
        await automation.build_automation(
            var.get_tool_call_start_trigger(),
            [(cg.std_string, "x")],
            config[CONF_ON_TOOL_CALL_START],
        )
