...
void ES8311::setup() {
  ...
  // Power On. Keep BIT(6) set when the codec is the I2S clock master.
  ES8311_ERROR_FAILED(this->write_byte(ES8311_REG00_RESET, this->force_master_ ? 0xC0 : 0x80));
}
...
void ES8311::dump_config() {
  ESP_LOGCONFIG(TAG,
                ...
                "  Use Microphone: %s\n"
                "  I2S Role: %s\n"
                "  MCLK multiple (codec): %" PRIu32 "  (MCLK ~= %" PRIu32 " Hz)\n"
                "  DAC Bits per Sample: %" PRIu8 "\n"
                "  Sample Rate: %" PRIu32,
                YESNO(this->use_mclk_), YESNO(this->use_mic_),
                this->force_master_ ? "MASTER (driving BCLK/LRCK)" : "SECONDARY (following BCLK/LRCK)",
                this->mclk_multiple_, this->sample_frequency_ * this->mclk_multiple_, this->resolution_out_,
  ...
  uint8_t reg31;
  uint8_t reg32;
  if (this->read_byte(ES8311_REG00_RESET, &reg00) && this->read_byte(ES8311_REG01_CLK_MANAGER, &reg01) &&
      this->read_byte(ES8311_REG02_CLK_MANAGER, &reg02) && this->read_byte(ES8311_REG03_CLK_MANAGER, &reg03) &&
      this->read_byte(ES8311_REG04_CLK_MANAGER, &reg04) && this->read_byte(ES8311_REG05_CLK_MANAGER, &reg05) &&
      this->read_byte(ES8311_REG06_CLK_MANAGER, &reg06) && this->read_byte(ES8311_REG07_CLK_MANAGER, &reg07) &&
      this->read_byte(ES8311_REG08_CLK_MANAGER, &reg08) && this->read_byte(ES8311_REG09_SDPIN, &reg09) &&
      this->read_byte(ES8311_REG0A_SDPOUT, &reg0a) && this->read_byte(ES8311_REG31_DAC, &reg31) &&
      this->read_byte(ES8311_REG32_DAC, &reg32)) {
    ESP_LOGCONFIG(TAG, "  REG00=0x%02X REG01=0x%02X REG02=0x%02X REG03=0x%02X", reg00, reg01, reg02, reg03);
    ESP_LOGCONFIG(TAG, "  REG04=0x%02X REG05=0x%02X REG06=0x%02X REG07=0x%02X REG08=0x%02X", reg04, reg05,
                  reg06, reg07, reg08);
    ESP_LOGCONFIG(TAG, "  REG09(SDPIN)=0x%02X REG0A(SDPOUT)=0x%02X  REG31(DAC mute)=0x%02X REG32(DAC vol)=0x%02X",
                  reg09, reg0a, reg31, reg32);
  }
}
...
bool ES8311::configure_format_() {
  // Configure I2S role and format. REG00 BIT(6) makes ES8311 drive BCLK/LRCK.
  uint8_t reg00;
